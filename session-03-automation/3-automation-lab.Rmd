...
```{r eval = FALSE}
geod_dist(lat1 = 49.5, lon1 = 8.4, lat2 = 52.5, lon2 = 13.4)
traceback() #traceback looks inside bugged function and returns the line where the error happened first
```
...
# load packages
library(tidyverse)
library(legislatoR)                                                                         # /!\ "LegislatoR" -> "legislatoR"

# get political data on German legislators
political_df <- 
  left_join(x = filter(get_political(legislature = "deu"), as.numeric("session") == 18),    # /!\ changed var name "ger" -> "deu"
            y = get_core(legislature = "deu"), by = "pageid") 




# wiki traffic data -- /!\ I changed nothing here bc it loads the data as such. I struggle to identify a bug, but the outout is lame
traffic_df <- 
...
# sessions served
sessions_served_df <- 
  get_political(legislature = "deu") |>                                                    # /!\ pipe operator was wrong 
  group_by(pageid) |> 
  dplyr::summarize(sessions_served = n())

# merge -- /!\ same comment as for traffic data
legislator_df <- 
