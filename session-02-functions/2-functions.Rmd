...
#just for the purposes of exo 4
...
get_mode <- function(v) { # Simon's choice
  out <- v |> 
    janitor::tabyl() |> 
    filter(n == max(n)) |> 
    pull(v)
...
get_mode_alexis <- function(v) {
  mode <- v |> 
    janitor::tabyl() |> 
    filter(n == max(n)) |> 
    pull(v)
#  freq <- mode |>     # an attempt to return the mode and the frequency of the mode. doesn't work like this
#    filter(n == max(freq)) |> 
 #   pull(freq)
  return(mode)
}

get_mode_alexis(penguins$bill_depth_mm) # it works

freq_table <- tabyl(penguins$bill_depth_mm)
...
```{r}
get_mode_alexis((penguins$flipper_length_mm))
```
:::
...
```{r}
purrr::map_dbl(df, mean, na.rm = TRUE) # we use doubles hence purrr::map_dbl()
```
...
```{r}
purrr::map2_dbl(df, df2, mean, na.rm = T)

purrr::map_dbl(df, mean, na.rm = T)
...
sumstats <- function(df) {
  ...
  mean <- purrr::map(df, mean, na.rm = TRUE)
  sd <- purrr::map(df, sd, na.rm = TRUE)
  var_names <- c(colnames(df))
