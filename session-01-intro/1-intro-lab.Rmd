...
#here str() short for structure
# factors are data falling into set values i.e a penguin belongs to either of three species
```
...
```{r, echo=F}
table(penguins$species, penguins$sex, useNA = "always") #useNA to add a distribution of NAs
...
```{r,, eval=F}
dplyr::select(penguins, species, island, rec_year, body_mass_g, sex)
```
...
```{r,, eval=F}
penguins |>
  select(species, sex, rec_year, body_mass_g) |> 
  #filter(!is.na(sex)) |> #just getting rid of propagated NAs in task with sex instead of year
  group_by(species, rec_year) |> 
  summarise(lightest_penguins = min(body_mass_g, na.rm = T)) |>
  ungroup()
...
```{r, eval=F}
penguins |>
  dplyr::arrange(desc(bill_length_mm)) #the pipe makes it unnecessery to provide a dataframe direction
...
```{r,, eval=F}
penguins |> # default ascending order
  filter(island == "Dream") |> 
  arrange(body_mass_g)

penguins |> # descending order with desc() argument within arrange()
...
```{r eval=F}
my_new_vector <- penguins |> #and save it as a vector
...
penguins |> dplyr::distinct(rec_year)
```
