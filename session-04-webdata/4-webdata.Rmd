...
words_changed <- str_replace_all(words, c("ing" = "er"))
# Your code here
<<<<<<< HEAD
words_changed <- str_replace_all(words, c("ing" = "er"))
...
>>>>>>> db086bce164228f273a25ecb6b987f285c046e2f
```
:::
...
count_words <- sum(str_detect(stringr::words, "ise"))
count_words
...
```{r}
unlist(str_extract_all(stringr::words, "tiny|sentence"))
```
...
```{r}
# Your code here
str_view(stringr::words, ".ing$|.ese$") |> unlist()
```
:::
...
```{r}
str_extract(example.obj, "s[[:alpha:]]{3}l") #an s followed by any 3 letters and then one l 
str_extract(example.obj, "A.+sentence") # here a greedy quantifier
```
...
```{r}
str_extract(example.obj, "A.+sentence") # the greedy gets everything in between A----- sentence
str_extract(example.obj, "A.+?sentence") #here we limit the greedy with ? which stops mqtchingh after 1 match of "sentence"
```
...
```{r}
# Your code here
str_view(stringr::words, "^[:alpha:]{2}y$") #just to work on the case before completing task with sum()
...
# also possible using str_split()
str_split(example.obj, pattern = "\\b") |> 
  str_subset([:alpha:]{1,5}) #just check solution
```
:::
