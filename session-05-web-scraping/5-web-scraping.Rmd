...
```{r}
parsed_url |> 
  rvest::html_element(xpath = '//p[76]') |> # extracts the 76th paragraph
  rvest::html_text()                        # print the scraped text


# this is bad pratice, not robust to any edits on wikipedia page. below are other methods
```
...
parsed_url |> 
  rvest::html_element(xpath = '//div[contains(@class, "mw-heading mw-heading3")]//h3[@id="Carnival"]/following::p') |>                # this is an XPath 
  rvest::html_text()
...
# YOUR ANSWER HERE
parsed_url |> 
  rvest::html_elements(xpath = '//*[@id="mw-content-text"]/div[1]/p[8]') # this works
...
#2a. 
spotify_table |> summarise(
  top_100_cutoff_in_billion = min(streams_billions)
)

#2b.
spotify_table |> 
  janitor::tabyl(artist_s) |> 
  arrange(desc(n)) |> 
  tibble()


#2b. with separating the main and featured artists

artists <- spotify_table |> 
  separate_rows(artist_s, sep = "\\s*(and|featuring|with)\\*") |> 
  group_by(artist_s) |> 
  mutate(cumulative_streams = sum(streams_billions)) |> 
  ungroup()
artists |> head(5)
```
